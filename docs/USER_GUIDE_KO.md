...
- **기존 가상화면 재사용**: 호환되는 기존 DeX 가상화면 사용
- **잠자기 방지 (`-w`)**: 실행 중 가상화면 활성 상태 유지
...
입력이 발생했다고 자동으로 다시 나타나지는 않습니다. 트레이 아이콘을
두 번 클릭하거나 트레이 메뉴에서 열기를 선택해 창을 복원합니다.
